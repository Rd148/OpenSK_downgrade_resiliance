...
impl Ctap1Command {
    ...
    fn process_register(
        ...
        let sk = private_key
            ...
            .map_err(|_| Ctap1StatusCode::SW_INTERNAL_EXCEPTION)?;
        let pk = sk.genpk();
        let key_handle = encrypt_to_credential_id(env, &private_key, &application, None, None, None)
            .map_err(|_| Ctap1StatusCode::SW_INTERNAL_EXCEPTION)?;
...
mod test {
    ...
    fn test_process_authenticate_check_only() {
        ...
        let key_handle = encrypt_to_credential_id(&mut env, &sk, &application, None, None, None).unwrap();
